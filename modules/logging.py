...
class logging():
    ...
    async def commandtask(self, ctx):
        passed = "<:uwuyes:519291795541065768>" if not ctx.command_failed else "<:uwuno:519291795285344265>"
        embed = discord.Embed(color=0x7289da, title=f"Command {passed}", description=f'```{ctx.message.clean_content}```')
        embed.set_author(name=ctx.author, icon_url=ctx.author.avatar_url_as(format="jpg"))
        ...
        async with aiohttp.ClientSession() as session:
            webhook = Webhook.from_url('https://canary.discordapp.com/api/webhooks/530571001138774037/q2i5bEI_g-CMPdlJgEFHwkoqjzCpF2cOMlilY8CNyYkU5iBukPDJRyOUwIpQK9ju5tfT', adapter=AsyncWebhookAdapter(session))
            await webhook.send(embed=embed, username=self.bot.user.name, avatar_url=self.bot.user.avatar_url)

    async def dmtask(self, ctx):
        embed = discord.Embed(color=0x7289da, title="Private message", description=f'```{ctx.message.clean_content}```')
        embed.set_author(name=ctx.author, icon_url=ctx.author.avatar_url_as(format="jpg"))
        embed.add_field(name="Author", value=f'{ctx.author}({ctx.author.id})', inline=False)
        async with aiohttp.ClientSession() as session:
            webhook = Webhook.from_url('https://canary.discordapp.com/api/webhooks/530571001138774037/q2i5bEI_g-CMPdlJgEFHwkoqjzCpF2cOMlilY8CNyYkU5iBukPDJRyOUwIpQK9ju5tfT', adapter=AsyncWebhookAdapter(session))
    ...
    async def on_message(self, message):
        ...
        if not ctx.command and not ctx.prefix and not ctx.guild and ctx.author != ctx.me:
