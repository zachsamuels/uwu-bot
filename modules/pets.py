...
beta_servers = [513888506498646052, 336642139381301249, 253716112292839425, 509925667241066498]
...
class pets:
    ...
    async def booster_left(self):
        ...
        while not self.bot.is_closed():
            ...
            rows = await self.bot.pool.fetchrow("SELECT user_id, pet_id, active_boosters, expire_time FROM user_boosters ORDER BY expire_time ASC LIMIT 1;")
            if not rows:
                continue
            ...
            await self.bot.pool.execute("DELETE FROM user_boosters WHERE user_id = $1 AND pet_id = $2", rows['user_id'], rows['pet_id'])
    ...
    async def adopt(self, ctx):
        async with self.bot.pool.acquire() as conn:
            ...
            e = discord.Embed(colour=0x7289da, description="""If you are interested in adopting a pet say which number. If you aren't say "Cancel" """)
            e.set_author(name="Adopt a pet!")
            e.add_field(name="[1] Wolf", value="uwus 500 This cuddly friend will bring hours of joy", inline=False)
            e.add_field(name="[2] Gold Fish", value="uwus 500 The OG pet", inline=False)
            e.add_field(name="[3] Birb", value="uwus 1000 *insert birb sounds*", inline=False)
            e.add_field(name="[4] Cat", value="uwus 1500 The purrfect pet for cat lovers", inline=False)
            e.add_field(name="[5] Doggo", value="uwus 1500 You won't have a rufftime with this great pet", inline=False)
            e.add_field(name="[6] Dragon", value="uwus 100,000 Takes all your uwus and burns em!", inline=False)
            e.add_field(name="[7] uwu", value="uwus 999,999,999 If you manage to get uwu you are a legend", inline=False)
    ...
    async def special(self, ctx):
        async with self.bot.pool.acquire() as conn:
            ...
            e.add_field(name="[1] Tiger", value="Level 15 uwus 200,000 Very pawsome animal", inline=True)
            e.add_field(name="[2] Lion", value="Level 20 uwus 250,000 Not the cutest animal...", inline=True)
            e.add_field(name="[3] Peacock", value="Level 30 uwus 250,000 Don't let the beauty get you off track",
                        inline=True)
            e.add_field(name="[4] Kitten", value="Level 50 uwus 1,000,000 My favorite animal :shrug:", inline=True)
            e.set_footer(text=f"You have {user_amount['uwus']} uwus")
            pet_embed = await ctx.send(embed=e)
            pet_nums = ['1', '2', '3', '4']
            ...
            if user_amount['uwus'] < cost or user_amount['current_level'] < level:
                await pet_embed.delete()
                return await ctx.send("You can't afford this pet or you aren't the required level.", delete_after=30)

            await conn.execute("""INSERT INTO spc_user_pets (user_id, pet_name, pet_type, gender) VALUES ($1, $2, $3, $4)""", ctx.author.id, animal, animal, gender)
            await pet_embed.delete()
    ...
    async def pets(self, ctx, special=None):
        async with self.bot.pool.acquire() as conn:
            pets = await conn.fetch("SELECT * FROM spc_user_pets WHERE user_id = $1", ctx.author.id)
            if special is None:
    ...
    async def cuddles(self, ctx, *, pet: int = None):
        async with self.bot.pool.acquire() as conn:
            ...
            if user_cooldown == -2:
                ...
                await conn.execute("UPDATE user_pets SET love = user_pets.love + $1 WHERE user_id = $2 AND pet_id = $3",
                                   love, ctx.author.id, pet)
                await cuddle.delete()
    ...
    async def play(self, ctx, pet: int = None):
        async with self.bot.pool.acquire() as conn:
            ...
            if user_cooldown == -2:
                ...
                msg = "Your pet loved playing!"
                play_msg = "Playing!"
                cuddle = await ctx.send(play_msg)
                await asyncio.sleep(5)
                if love == 0:
                    msg = f"Your pet didn't like playing."
                if chance_of_item < 30 and love != 0:
                    items = ['2x XP', '2x XP', '2x uwus', '2x uwus', '3x XP', '3x uwus', '4x XP', '5x XP']
                    item = choice(items)
                    msg = f"Your pet enjoyed the playing and found a {item} booster (Note: All boosters only last 24 hours)!"
                    did_insert = await self.bot.pool.fetchval("""INSERT INTO pet_boosters (pet_id, booster_name)
SELECT $1, $2
WHERE (SELECT COUNT(*) FROM pet_boosters WHERE pet_id = $1) < 5
RETURNING True;""", pet, item)
                    if not did_insert:
                        msg = "You earned a booster but already have 5."
                await conn.execute("UPDATE spc_user_pets SET love = spc_user_pets.love + $1 WHERE user_id = $2 AND pet_id = $3", love, ctx.author.id, pet)
                await cuddle.delete()
                await ctx.send(f"{msg} You gained {love} loves!")
            else:
                base_time = user_cooldown / 1000
    ...
    async def booster(self, ctx):
        await ctx.send("No subcommand passed. Valid subcommands `activate, sell, and list`", delete_after=30)
    ...
    async def list(self, ctx, pet: int = None):
        ...
        await ctx.send(f"""Boosters for your pet with ID {pet} are `{", ".join(list_boosters)}`.""")
    ...
    async def sell(self, ctx, pet: int = None):
        async with self.bot.pool.acquire() as conn:
            user_cooldown = await self.bot.redis.pttl(f"{ctx.author.id}-{ctx.command.qualified_name}")
            if user_cooldown == -2:
                await self.bot.redis.execute("SET", f"{ctx.author.id}-{ctx.command.qualified_name}", "cooldown", "EX",
                                             7)
                ...
                if xp_from:
                    ...
                    await list_embed.delete()
                    return await ctx.send(f"Sold your {option} for {uwus} uwus and {xp_from} xp.")

                await conn.execute("UPDATE user_stats SET uwus = user_stats.uwus + $1 WHERE user_id = $2", uwus, ctx.author.id)
                await list_embed.delete()
    ...
    async def activate(self, ctx, pet: int = None, *, booster_type=None):
        async with self.bot.pool.acquire() as conn:
            ...
            if booster_type is None or booster_type.lower() not in boosters_lower:
                return await ctx.send(f"""Invalid booster type. Valid `{", ".join(boosters)}`""")
            booster_left = await conn.fetchrow(f"SELECT expire_time, active_boosters FROM user_boosters WHERE user_id = $1", ctx.author.id)
...
pet, booster_type, boost_amount, boost_type, ctx.author.id, end_time)
