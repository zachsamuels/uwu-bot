...
class patron:
    ...
    async def __local_check(self, ctx):
        if await self.bot.pool.fetchrow("SELECT user_id FROM p_users WHERE user_id = $1", ctx.author.id):
            return True

        raise(errorhandler.NotPatron(ctx))

    async def p_has_timer(self,user_id):
        user = await self.bot.pool.fetchrow("SELECT next_time FROM p_user_timer WHERE user_id = $1;",user_id)
        if user is not None:
    ...
    async def patron(self, ctx):
        await ctx.send("No subcommand passed or invalid was passed. Valid subcommands `timecheck, biweekly`")

    @patron.command(description="Check how long you have been a Patron", brief="Check how long you have been a patron")
    async def timecheck(self,ctx):
        patron = await self.bot.pool.fetchrow("SELECT patron_time FROM p_users WHERE user_id = $1", ctx.author.id)
    ...
    @patron.command(description="Patron only. Claim your biweekly uwus", brief="Claim your biweekly 5k uwus.")
