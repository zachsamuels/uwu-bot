...
class uwulonian:
    ...
    async def profile(self, ctx):
        async with self.bot.pool.acquire() as conn:
            ...
            async with ctx.typing():
                await ctx.send(file=discord.File(await self.bot.loop.run_in_executor(None, self.do_profile, ctx.author, author_avy, uwulonian_name), filename=f"{ctx.author.id}_prof.png"))
    ...
    async def color(self, ctx):
        await ctx.send("No subcommand passed. Valid subcommands `set, default, current, random, and list`", delete_after=30)
    ...
    async def list(self, ctx):
        await ctx.send(f"""Some valid colors are `{", ".join(extras.pil_colors)}`. A full list of colors can be found here <https://www.uwu-bot.xyz/color/>""")

    @color.command()
    async def random(self, ctx):
        rand_color = choice(extras.all_pil_colors)
        await self.bot.pool.execute("UPDATE user_settings SET profile_color = $1 WHERE user_id = $2", rand_color, ctx.author.id)
        await ctx.send(f"Your profile is now `{rand_color}`")

    @color.command()
    async def current(self, ctx):
        current_color = await self.bot.pool.fetchrow("SELECT profile_color FROM user_settings WHERE user_id = $1", ctx.author.id)
        await ctx.send(f"Your profiles current color is `{current_color['profile_color']}`")

    @color.command()
    async def default(self, ctx):
        await self.bot.pool.execute("UPDATE user_settings SET profile_color = $1 WHERE user_id = $2", 'white', ctx.author.id)
        await ctx.send("Set your profiles color to `white`")

    @commands.command(description='Get an uwulonians or your stats', aliases=['bal', 'wallet'], brief="Check a users stats.")
    async def stats(self, ctx, user: discord.Member=None):
        async with self.bot.pool.acquire() as conn:
            ...
            e.add_field(name=f"Stats for {uwulonian_name['user_name']}", value=f"""Foes killed - {uwulonian_name['foes_killed']}\nDeaths - {uwulonian_name['total_deaths']}\nuwus - {uwulonian_name['uwus']}""")
            e.add_field(name="Levels", value=f"XP - {uwulonian_name['current_xp']}\n Level - {uwulonian_name['current_level']}")
    ...
    @commands.command(aliases=['lb','wowcheaterhenumber1onlb'], description="Check the leaderboard for total_deaths, foes_killed, uwus, and current_xp, or current_level.", brief="Check leaderboards")
    async def leaderboard(self,ctx,sort=None):
        ...
        if sort.lower() == "deaths":
            ...
            new_sort = "uwus"
        lb = await self.bot.pool.fetch(f"SELECT user_settings.user_name, user_stats.{new_sort} FROM user_stats INNER JOIN user_settings ON  user_stats.user_id = user_settings.user_id ORDER BY {new_sort} DESC LIMIT 5;")
        e = discord.Embed(colour=0x7289da)
        num = 0
        e.set_author(name=f"Leaderboard - {sort.lower()}")
        for i in lb:
            e.add_field(name=f"#{num + 1}. {lb[num]['user_name']}", value=f"{sort.lower()} - {lb[num][new_sort]}",inline=False)
            num += 1
        await ctx.send(embed=e)
    ...
    async def levelup(self, ctx):
        async with self.bot.pool.acquire() as conn:
            user = await conn.fetchrow("SELECT * FROM user_settings INNER JOIN user_stats ON user_settings.user_id = user_stats.user_id WHERE user_settings.user_id = $1 AND user_stats.user_id = $1", ctx.author.id)
            ...
            await ctx.send(f"{user['user_name']} is now level {new_level}!")
