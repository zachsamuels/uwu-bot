...
class exploring:
    ...
    async def explore_waiter(self):
        ...
        while not self.bot.is_closed():
            ...
            rows = await self.bot.pool.fetchrow("SELECT user_explores.user_id, finish_time, user_stats.uwus, user_stats.current_level, user_stats.current_xp FROM user_explores INNER JOIN user_stats ON user_explores.user_id = user_stats.user_id ORDER BY finish_time ASC LIMIT 1;")
            if not rows:
                continue
...
""")
            ...
            try:
                ...
                pass
            await self.bot.pool.execute("UPDATE user_stats SET uwus = user_stats.uwus + $1, current_xp = user_stats.current_xp + $2, foes_killed = user_stats.foes_killed + $3, total_deaths = user_stats.total_deaths + $4 WHERE user_id = $5", uwus, xp, foes, deaths, user.id)
            await self.bot.pool.execute("DELETE FROM user_explores WHERE user_id = $1", rows['user_id'])

    async def adventure_waiter(self):
        await self.bot.wait_until_ready()
        while not self.bot.is_closed():
            await asyncio.sleep(2)
            rows = await self.bot.pool.fetchrow("SELECT user_adventures.user_id, finish_time, user_stats.uwus, user_stats.current_level, user_stats.current_xp FROM user_adventures INNER JOIN user_stats ON user_adventures.user_id = user_stats.user_id ORDER BY finish_time ASC LIMIT 1;")
            if not rows:
                continue
            await extras.sleep_time(rows['finish_time'])
            user = self.bot.get_user(rows['user_id'])
...
""")
    ...
    async def explore(self, ctx):
        async with self.bot.pool.acquire() as conn:
            explore = await conn.fetchrow("SELECT * FROM user_explores WHERE user_id = $1", ctx.author.id)
            adventure = await conn.fetchrow("SELECT * FROM user_adventures WHERE user_id = $1", ctx.author.id)
            if explore:
